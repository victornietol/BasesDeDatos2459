-- Tablas
drop database proyecto_bd1_2459_prueba;
CREATE DATABASE IF NOT EXISTS proyecto_bd1_2459_prueba;
...
delimiter //
DROP TRIGGER IF EXISTS bi_provincia //
CREATE TRIGGER bi_provincia
...
BEGIN
    ...
    if(new.codigo < 0) then
		signal sqlstate'45000'
        set message_text = 'Código inválido';
	end if;
end; //
delimiter ;
...
FOR EACH ROW
BEGIN
	if(new.num_linea > 9999) then
		signal sqlstate'45000'
        set message_text = 'El número de linea excede el número de caracteres permitido';
	end if;
END; //
delimiter ;
...
select z.nombre zona, p.nombre provincia, p.codigo codigo
from zona z JOIN provincia p ON (z.id_provincia = p.id_provincia)
group by zona;

CREATE OR REPLACE VIEW informacion_consumidor AS
select z.nombre zona, c.nombre tipo_consumidor, ic.consumo_medio consumo_med, ic.num_consumidores n_consumidores
from info_consumidor ic
JOIN (select zo.id_zona id_zona, zo.nombre from zona zo JOIN info_consumidor i ON (zo.id_zona = i.id_zona) ) z ON (ic.id_zona = z.id_zona)
JOIN (select cc.nombre, cc.id_consumidor id_consumidor from consumidores_cat cc JOIN info_consumidor i ON (cc.id_consumidor = i.id_consumidor) ) c ON (ic.id_consumidor = c.id_consumidor);